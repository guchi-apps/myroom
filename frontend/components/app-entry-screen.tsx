 ...
 * 起動直後の画面はこれを出す。
 *
 * **絵柄を持たず、生成済みの `public/kurashio-icon-192.png` をそのまま表示する。**
 * アイコンの正は `assets/kurashio-app-icon.png`（CLAUDE.md「アプリアイコン」）で、
 * ここに絵柄を書き写すと描き直したときの更新漏れが増えるため。
...
export function AppEntryScreen({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-background p-6 text-center">
      ...
      <p className="mt-1.5 text-sm text-muted-foreground">暮らしを、ひとつに整える。</p>
