...
- **画面から書き換える設定を足すだけなら、テーブルも列も増やさなくてよい。** `app_settings` は
  `setting_key` / `setting_value`（JSON文字列）のキーバリュー表で、`backend/ui_settings.py` に
  キーと正規化関数を1つ足せば済む。**DDLを含まないのでマイグレーション専用ユーザーも要らない**
  （#262 の `remote_button_defs` / `remote_catalog` がこの形）。`setting_value` は `Text`（64KB）なので、
  数百KBになるものだけは別の置き場を考える
